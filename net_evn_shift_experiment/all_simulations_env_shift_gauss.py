...
for i in xrange(N_SIM):
    for network in networks+networks2:
        res_fname = "results_of_simulation/env_shift_gauss_sim_results/results_{0}{1}".format(network, i)
        if not os.path.isfile(res_fname):
            pool.apply_async(os.system, ["python2 cog_simulations/steels/steels_main.py -s examples/simulations/env_shift_gauss_simulations/simulation_env_shift_gauss_training_{0}.json -r {1}".format(network, res_fname)])
...
for i in xrange(N_SIM):
    for result in results:
        for network in networks+networks2:
            res_fname = "results_of_simulation/env_shift_gauss_sim_data/data_env_training{0}{2}_{1}".format(network, i, result)
            if not os.path.isfile(res_fname):
                pool.apply_async(os.system, ["python2 cog_simulations/steels/analyzer.py -r results_of_simulation/env_shift_gauss_sim_results/results_{0}{1} it {2} > {3}".format(network, i, result, res_fname)])
    ...
    for network in networks+networks2:
        ...
        for i in xrange(N_SIM):
            data_sim = pd.read_csv(
                "results_of_simulation/env_shift_gauss_sim_data/data_env_training{0}{1}_{2}".format(network, result, i),
...
with open("env_shift_gauss_sim_results.csv", 'w') as f:
    data.to_csv(f)
